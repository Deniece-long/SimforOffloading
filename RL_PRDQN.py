...
class SumTree(object):
    """
    Describe the data with the priority in the tree
    """
    data_pointer = 0
...
class Memory(object):  # stored as ( s, a, r, s_ ) in SumTree
    ...
    def sample(self, n):
        ...
        for i in range(n):
            ...
            if min_prob == 0:
                ISWeights[i, 0] = np.power(prob / 0.0001, -self.beta)
            else:
...
class PRDQN(object):
    def __init__(
        ...
        self.prioritized = True  # decide to use double q or not
    ...
    def learn(self):
        ...
        if self.prioritized:
            _, abs_errors, self.loss_ = self.sess.run([self._train_op, self.abs_errors, self.loss],
    ...
    def plot_lost(self, name='PRDQN', userNum='5'):
